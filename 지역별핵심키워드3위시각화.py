...
# 지역기사만 추출하고 상세지역만 상세지역컬럼 만들어서 넣어둠
...
df_region['키워드'] = df_region['키워드'].fillna('')  # 있던 컬럼에 수정해서 덮어쓰기
# 빈문자열이라도 삽입
...
def get_clean_keywords(text):
    ...
    for word in words:
        ...
        if any(region in word for region in all_region_names): continue   # continue를 작성함으로써 필요없는 데이터를 실행되지 않아야지만 기회가 넘어가서 실행이 된다.
...
visual_df = df_region.groupby('상세지역').agg(           # 컬럼이 총 3개 탄생(상세지역,뉴스건수,키워드순) 
    뉴스건수=('상세지역', 'count'),
...
visual_df['상위3개키워드'] = visual_df['키워드순'].apply(lambda x: ', '.join(x[:3]))   # 정제된 리스트를 X로 넘겨서 상위 3개로 짤라서 하나으로 뭉쳐서 쉼표 써서 문자열로 만들어라는 뜻  join => 문자열 
# 상위3개키워드 컬럼 추가하고 키워드순 리스트로 값이 되어있는데 하나의 문자열로 쉽표가 포함된 3자리까지 합쳐서 넣어주기 (join)

plt.figure(figsize=(14, 7))


ax = sns.barplot(       # sns: seabon (막대그래프)
    data=visual_df.reset_index(),        # reset_index(),  => 키가 제일 큰 애를 앞으로 보내라는 뜻
    x='상세지역', 
    y='뉴스건수', 
    hue='상위3개키워드', 
